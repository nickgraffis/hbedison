...
const validParentEmail = ref(false)
const validParentName = ref(false)
const validAthleteName = ref(false)
const validAthleteEmail = ref(false)
...
const updateParentEmail = (event: any) => {
  const validate = checkEmails(event.target.value)
  if (!validate) validParentEmail.value = false
  else validParentEmail.value = true
  parentEmail.value = event.target.value
}

const athleteEmail = ref('')
const updateAthleteEmail = (event: any) => {
  const validate = checkEmails(event.target.value)
  if (!validate) validAthleteEmail.value = false
  else validAthleteEmail.value = true
  athleteEmail.value = event.target.value
}

const parentName = ref('')
const updateParentName = (event: any) => {
  validParentName.value = true
  parentName.value = event.target.value
}

const athleteName = ref('')
const updateAthleteName = (event: any) => {
  validAthleteName.value = true
  athleteName.value = event.target.value
}

const processForm = (event?: Event) => {
  event?.preventDefault()
  if (!parentEmail.value || !validParentEmail.value) return
  ...
  fetch(`https://edisonwp.com/.netlify/functions/summer-signup`, {
    method: 'POST',
...
const updateIsAthlete = () => {
  isAthlete.value = !isAthlete.value
}
      ...
      <div class="relative px-6 py-10 overflow-hidden bg-green-700 border-4 border-green-700 shadow-xl rounded-2xl sm:px-12">
        ...
        <div class="relative">
          ...
          <p class="max-w-2xl mx-auto mt-4 text-sm lg:text-base text-yellow-50">
            Add an email to stay up to date with Edison Water Polo leading into Summer 2023. ⚡️
          </p>
          <form class="mt-6 mx-auto sm:max-w-lg" @submit="processForm">
            <div class="relative flex-1 min-w-0 space-y-3">
              <div class="flex space-x-2 w-full">
                <div class="flex items-center mr-4 mb-2 cursor-pointer flex-grow" @change="updateIsAthlete()">
                  ...
                  <label class="text-base lg:text-lg text-yellow-50 font-semibold">Athlete Sign Up</label>
                </div>
                <div class="flex items-center mr-4 mb-2 cursor-pointer flex-grow" @click="updateIsParent()">
                  <input type="checkbox" class="opacity-0 absolute h-8 w-8 cursor-pointer">
                  <div class="border-white border-2 cursor-pointer rounded-md w-8 h-8 flex flex-shrink-0 justify-center items-center mr-2">
                    <svg :class="!isParent && 'hidden'" class="fill-current w-3 h-3 text-white" version="1.1" viewBox="0 0 17 12" xmlns="http://www.w3.org/2000/svg">
                      <g fill="none" fill-rule="evenodd">
                        <g transform="translate(-9 -11)" fill="#fff" fill-rule="nonzero">
                          <path d="m25.576 11.414c0.56558 0.55188 0.56558 1.4439 0 1.9961l-9.404 9.176c-0.28213 0.27529-0.65247 0.41385-1.0228 0.41385-0.37034 0-0.74068-0.13855-1.0228-0.41385l-4.7019-4.588c-0.56584-0.55188-0.56584-1.4442 0-1.9961 0.56558-0.55214 1.4798-0.55214 2.0456 0l3.679 3.5899 8.3812-8.1779c0.56558-0.55214 1.4798-0.55214 2.0456 0z" />
                        </g>
                  ...
                  <label class="text-base lg:text-lg text-yellow-50 font-semibold">Parent Sign Up</label>
                </div>
              </div>
              <div class="py-1" v-if="isParent">
                <label for="parentName" class="text-white">Parent Name</label>
                <input
                  id="parentName"
                  :value="parentName"
                  name="parentName"
                  type="text"
                  :class="[validParentName ? 'focus:ring-offset-green-600 ' : 'focus:ring-offset-red-600']"
                  ...
                  placeholder="Nick Graffis"
                  @blur="touched"
                  @keyup="updateParentName"
                />
              </div>
              <div class="py-1" v-if="isParent">
                <label for="parentEmail" class="text-white">Parent Email Addresses</label>
                <input
                  id="parentEmail"
                  :value="parentEmail"
                  name="parentEmail"
                  type="text"
                  :class="[validParentEmail ? 'focus:ring-offset-green-600 ' : 'focus:ring-offset-red-600']"
                  class="block w-full px-5 py-3 text-sm lg:text-base text-gray-900 placeholder-gray-500 border border-transparent rounded-md shadow-sm focus:outline-none focus:border-transparent focus:ring-white focus:ring-2 focus:ring-offset-2"
                  placeholder="edison@polo.com, nick@nick.com "
                  @blur="touched"
                  @keyup="updateParentEmail"
                />
                <p class="text-xs text-gray-100">
                  Enter as many email addresses as you would like, seperate them by comma. Example: edison@polo.com, nick@nick.com
                </p>
              </div>
              <div class="py-1" v-if="isParent || isAthlete">
                <label for="athleteName" class="text-white">Athlete Name</label>
                <input
                  id="athleteName"
                  :value="athleteName"
                  name="athleteName"
                  type="text"
                  :class="[validAthleteName ? 'focus:ring-offset-green-600 ' : 'focus:ring-offset-red-600']"
                  class="block w-full px-5 py-3 text-sm lg:text-base text-gray-900 placeholder-gray-500 border border-transparent rounded-md shadow-sm focus:outline-none focus:border-transparent focus:ring-white focus:ring-2 focus:ring-offset-2"
                  placeholder="Nick Graffis"
                  @blur="touched"
                  @keyup="updateAthleteName"
                />
              </div>
              <div class="py-1" v-if="isAthlete">
                <label for="athleteEmail" class="text-white">Athlete Email Addresses</label>
                <input
                  id="athleteEmail"
                  :value="athleteEmail"
                  name="athleteEmail"
                  type="text"
                  :class="[validAthleteEmail ? 'focus:ring-offset-green-600 ' : 'focus:ring-offset-red-600']"
                  class="block w-full px-5 py-3 text-sm lg:text-base text-gray-900 placeholder-gray-500 border border-transparent rounded-md shadow-sm focus:outline-none focus:border-transparent focus:ring-white focus:ring-2 focus:ring-offset-2"
                  placeholder="edison@polo.com, nick@nick.com "
                  @blur="touched"
                  @keyup="updateAthleteEmail"
                />
                <p class="text-xs text-gray-100">
                  Enter as many email addresses as you would like, seperate them by comma. Example: edison@polo.com, nick@nick.com
                </p>
            ...
            <div class="mt-4 flex items-center justify-center">
              <button :disabled="(isParent && (!validParentEmail || !athleteName || !parentName)) || (isAthlete && (!validAthleteEmail || !athleteName))" type="submit" class="inline-flex justify-center items-center w-full px-5 py-3 text-sm lg:text-base font-medium text-gray-100 bg-green-500 border border-transparent rounded-md shadow hover:bg-green-400 focus:outline-none sm:px-10 disabled:bg-green-400 disabled:cursor-not-allowed">
          ...
          <p v-if="isParent && !validParentEmail && beenTouched" class="flex items-start space-x-2 text-sm font-semibold text-gray-100">
          ...
          <p v-if="isParent && !parentName || !athleteName && beenTouched" class="flex items-start space-x-2 text-sm font-semibold text-gray-100">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 flex-shrink-0 text-red-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
            <span>Please enter Parent and Athelte name.</span>
          </p>
          <p v-if="isAthlete && !validAthleteEmail && beenTouched" class="flex items-start space-x-2 text-sm font-semibold text-gray-100">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 flex-shrink-0 text-red-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
            <span>Please enter valid email(s), seperated by commas.</span>
